...
public struct PostTextParser {
  ...
  /// Changed to allow zero forward slashes after first colon, because magnet links don't
  /// use any forward slashes.
  let urlRegEx = try! NSRegularExpression(
    pattern: #"(?i)\b((?:[a-z][\w-]+:(?:/{0,3}|[a-z0-9%])|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'".,<>?«»“”‘’]))"#,
    options: [])
    ...
    textCoalescingTokenizer(text: text) {
      ...
      case .text(let text):
        if let context = tagStack.last {
          ...
          let matches = urlRegEx.matches(in: text, options: [], range: nsrange)
